  getDashboard: async (projectId?: string) => {
    try {
      ...
      return await request<DeliveryDashboardData>(`/delivery/dashboard${query}`);
    } catch {
      // Mock fallback for static deployment (Vercel)
      return {
        total_plans: 5,
        draft: 1,
        at_risk: 2,
        ...
        total_risks: 14,
        high_risks: 3,
        risk_health: 'yellow' as const,
        delivery_health: 'green' as const,
        total_tasks: 50,
        completed_tasks: 28,
